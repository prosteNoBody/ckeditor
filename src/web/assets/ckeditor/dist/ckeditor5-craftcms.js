...
import { ImageInsertUI, ButtonView, icons, Plugin, LinkUI, ContextualBalloon, createDropdown, SplitButtonView, addListToDropdown, Collection, ViewModel, Range, Command, ImageUtils, DropdownButtonView, Widget, viewToModelPositionOutsideModelElement, toWidget, DomEventObserver, WidgetToolbarRepository, isWidget } from "ckeditor5";
/**
      ...
      onBeforeSubmit: async () => {
        ...
        if ($element !== null && Garnish.hasAttr($element, "data-owner-is-canonical") && !elementEditor.settings.isUnpublishedDraft) {
          ...
          if (elementEditor.settings.draftId && slideout.elementEditor.settings.draftId) {
            ...
            slideout.elementEditor.settings.saveParams.newOwnerId = elementEditor.getDraftElementId($element.data("owner-id"));
          }
        }
      },
      onSubmit: (ev) => {
        ...
        if ($element !== null && ev.data.id != $element.data("id")) {
          $element.attr("data-id", ev.data.id).data("id", ev.data.id).data("owner-id", ev.data.ownerId);
...
let localizedRefHandles = [];
function setLocalizedRefHandles(handles) {
...
const findPlugin = (pluginName) => {
  for (const [_, v] of Object.entries(CKEditor5)) {
    if (typeof v === "object") {
      for (const [_2, v2] of Object.entries(v)) {
...
const registerPackage = (pkg) => {
  ...
  if (pkg.pluginNames && pkg.pluginNames.length && pkg.toolbarItems && pkg.toolbarItems.length) {
    pluginButtonMap.push({
      plugins: pkg.pluginNames,
      buttons: pkg.toolbarItems.flat().map((item) => item.button).flat()
    });
  }
};
...
const create = async function(element, config) {
  ...
  if (config.toolbar) {
    removePlugins.push(
      ...pluginButtonMap.filter(
        ({ buttons }) => !config.toolbar.items.some((button) => buttons.includes(button))
      ).map(({ plugins: plugins2 }) => plugins2).flat()
    );
  }
